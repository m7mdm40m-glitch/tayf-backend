...
import { getUser, createUser, addCredits, deductCredits, setCredits, updateUser, getUserByVerifyToken, getUserByRefCode, incrementRefCount, setResetCode, resetPasswordWithCode, addCreation, getCreations, deleteCreation, toggleFavorite, adminGetAllUsers, adminGetStats, adminDeleteUser } from "./store.js";
...
// مكافأة الإحالة (رصيد لكل طرف عند تفعيل المدعو)
const REFERRAL_BONUS = Number(process.env.REFERRAL_BONUS || 3);
// توليد كود إحالة قصير فريد
function genRefCode() {
  return crypto.randomBytes(5).toString("base64").replace(/[^a-zA-Z0-9]/g, "").slice(0, 7).toUpperCase();
}
...
app.post("/api/signup", authLimiter, async (req, res) => {
  ...
  // إحالة: من دعا هذا المستخدم (إن وُجد كود صالح ولم يُحل نفسه)
  let referredBy = null;
  if (ref) {
    try {
      const referrer = await getUserByRefCode(String(ref).trim().toUpperCase());
      if (referrer && referrer.email !== key) referredBy = referrer.email;
    } catch (e) { console.error("ref lookup error", e?.message); }
  }
  // كود إحالة خاص بالمستخدم الجديد (فريد)
  let refCode = genRefCode();
  for (let tries = 0; tries < 3; tries++) {
    try { if (await getUserByRefCode(refCode)) refCode = genRefCode(); else break; } catch { break; }
  }

  await createUser({ name: name.trim(), email: key, passHash, credits: 0, verified: false, verifyToken, refCode, referredBy });
...
app.get("/api/verify", async (req, res) => {
  ...
  // مكافأة الإحالة: المدعو يأخذ المجاني + مكافأة، والداعي يأخذ مكافأة (مرّة واحدة عند التفعيل)
  let newUserCredits = FREE_CREDITS;
  if (u.referredBy) {
    try {
      const referrer = await getUser(u.referredBy);
      if (referrer) {
        newUserCredits = FREE_CREDITS + REFERRAL_BONUS;
        await addCredits(referrer.email, REFERRAL_BONUS);
        await incrementRefCount(referrer.email);
      }
    } catch (e) { console.error("referral bonus error", e?.message); }
  }
  await updateUser(u.email, { verified: true, verifyToken: null, credits: newUserCredits });
...
// معلومات الإحالة: كود المستخدم وعدد إحالاته (يولّد كوداً للحسابات القديمة)
app.get("/api/referral", auth, async (req, res) => {
  try {
    let code = req.user.refCode;
    if (!code) {
      code = genRefCode();
      for (let tries = 0; tries < 3; tries++) {
        const exists = await getUserByRefCode(code);
        if (exists) code = genRefCode(); else break;
      }
      await updateUser(req.user.email, { refCode: code });
    }
    res.json({ code, count: req.user.refCount || 0, bonus: REFERRAL_BONUS });
  } catch (e) {
